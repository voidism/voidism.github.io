...
現在common mode的增益就不是0了，我們想知道對於common mode的$G_m$值是多少，稱作$G_{mcm}$(在前一節是不需要討論這個的)，我們用類似半電路的方式來看他，忽略電晶體上的小電阻$\frac{1}{g_m}$之後，整條電路上就只剩下$2R_{SS}$，因此$G_{mcm} = \frac {1}{2R_{SS}}$，看單一半電路的$R_O$為 $2R_{SS}+r_{o}+(g_mr_o)(2R_{SS})$。

現在要求$A_{cm}$，由於左半電路的$v_g$跟$v_d$是接在一起的，上面的電晶體就可以視為$\frac{1}{g_m}$的小電阻，$v_d$可算出$- G_{mcm}v_{icm}(R_o \Vert r_{o3} \Vert 1/g_{m3})$，由於$A_{cm}$是個蠻小的數字，所以我們先不能亂省東西，
