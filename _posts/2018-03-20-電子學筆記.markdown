...
因此，我們定義$V_{OS} = V_O/A_d$，對於MOS來說，針對$R_D$的偏差，我們算出$V_{OS} = (\frac {V_{OV}}{2})(\frac{\Delta R_D}{R_D})$；針對$(W/L)$的偏差，我們算出$V_{OS} = (\frac {V_{OV}}{2})(\frac{\Delta (W/L)}{(W/L)})$；針對$V_t$的偏差則會直接影響到$V_{OV}$，結果很簡單，我們算出$V_{OS} = \Delta V_t$。由於這三種因素是independent，total的$V_{OS}$可以估計成$V_{OS} = \sqrt {(\frac {V_{OV}}{2} \frac{\Delta (W/L)}{(W/L)})^2 + (\frac {V_{OV}}{2}\frac{\Delta (W/L)}{(W/L)})^2 + (\Delta V_t)^2}$。
...
所以BJT total的$V_{OS}$可以估計成$V_{OS} = \sqrt {( V_T \frac{\Delta (W/L)}{(W/L)})^2 + (V_T \frac{\Delta (W/L)}{(W/L)})^2 }$。
