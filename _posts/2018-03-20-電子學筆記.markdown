...
，其實這就跟MOS的公式87%像：$i_{D1} = \dfrac {I}{2} - \dfrac {I}{2V_{OV}}\dfrac {v_{id}}{2}$，可以看到BJT除了比MOS乘上$\alpha$倍以外，等於是用了$V_T$去取代$V_{OV}/2$。而如果不看共有的電流，只看增加的電流(也就是小信號)，得到$i_{c} = \dfrac {\alpha I}{2V_T}\dfrac {v_{id}}{2}$，剛好$g_m = I_C/V_T$，因此$i_{c} = g_m\dfrac {v_{id}}{2}$。
...
原本在MOS中，輸入電阻是無限大，但現在因為是BJT，所以我們要考慮它輸入differential信號時的電阻$R_{id}$
